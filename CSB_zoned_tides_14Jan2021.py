...
os.chdir(directory)
fp_zones = r"G:\csb\tide zone polygons\tide_zone_polygons.shp"
...
def CorrectTides():
    ...
    for filepath in files:
        ...
        #populate vessel name data from json file metadata header
        y=open(filepath)
        x=json.load(y)
        ...
        csb_corr = csb_corr.drop(columns=['index_right','ATCorr','RR','ATCorr2','RR2','DataProv','Shape_Leng','Shape_Area','Shape_Le_1','t','v','s','f','q','t_corr','t_new','v_new'])
        print(csb_corr)
        try:   
            csb_corr.to_file('G:/csb/shapefiles/vero beach/csb_'+ title +'.shp', driver='ESRI Shapefile')
        except Exception:
            pass
